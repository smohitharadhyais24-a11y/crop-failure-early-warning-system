...
function App() {
  ...
  const handleAnalyze = async (values) => {
    console.log('[APP] handleAnalyze called with values:', values);
    setLoading(true);
    setError(null);
    setPrediction(null);

    try {
      console.log('[APP] Sending prediction request...');
      ...
      console.log('[APP] Prediction response received:', response.data);
      setPrediction(response.data);
      setSelectedValues(values);
      console.log('[APP] State updated with prediction data');
    } catch (err) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="px-4 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg transition text-sm font-semibold"
            >
              ← Back to Home
            </button>
          </div>
